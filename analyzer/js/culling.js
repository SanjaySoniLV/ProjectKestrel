    ...
    async function writeMetadataForFolder(rootPath) {
      ...
      // Adobe Lightroom ignores .xmp sidecars for JPEGs — it only reads XMP
      // embedded inside the file. Offer an opt-in to embed directly into JPEG
      // originals, but only when the folder actually contains JPEGs.
      const jpegCount = folderRows.filter(r => /\.jpe?g$/i.test(r.filename || '')).length;
      const hasJpeg = jpegCount > 0;
      ...
      // Opt-in section for embedding XMP directly into JPEG originals. Only
      // shown when the folder contains JPEGs. Default OFF (unchecked) every
      // time — this modifies the user's original files, so it must be a
      // conscious choice, never a sticky default.
      const embedSection = hasJpeg ? `
          <div style="background:#211a10;border:1px solid #4a3820;border-radius:8px;padding:11px 14px;margin-bottom:12px;">
            <label style="display:flex;align-items:flex-start;gap:9px;cursor:pointer;">
              <input type="checkbox" id="wmEmbedJpeg" style="margin-top:2px;flex-shrink:0;" />
              <span style="display:flex;flex-direction:column;gap:3px;min-width:0;">
                <span style="font-size:12px;font-weight:700;color:#e8c98a;">Also embed metadata into JPEG originals <span style="font-weight:600;color:#9a8050;">(${jpegCount} JPEG${jpegCount === 1 ? '' : 's'})</span></span>
                <span style="font-size:11px;color:#b89b6a;line-height:1.45;">Adobe Lightroom ignores <code style="background:#1c2438;padding:1px 4px;border-radius:3px;">.xmp</code> sidecars for JPEGs — embedding writes the ratings and tags <b>inside</b> each JPEG so Lightroom can read them.</span>
                <span style="font-size:11px;color:#e0a050;line-height:1.45;">&#9888; This <b>modifies your original JPEG files</b> in place. Photo data is left untouched, but as with any file edit, use at your own risk — back up if unsure. RAW files are never modified.</span>
              </span>
            </label>
          </div>` : '';
      ...
      dlg.querySelector('#wmOk').addEventListener('click', async () => {
        ...
        const embedJpeg = !!dlg.querySelector('#wmEmbedJpeg')?.checked;
        // Human-readable suffix describing embedded JPEG results, appended to
        // the sidecar-count messaging when embedding was requested.
        const embedSuffix = (r) => {
          if (!embedJpeg) return '';
          let s = r && r.embedded ? `, ${r.embedded} embedded in JPEG${r.embedded === 1 ? '' : 's'}` : '';
          if (r && r.embed_errors && r.embed_errors.length) s += `, ${r.embed_errors.length} embed failed`;
          return s;
        };
        showView('wmProgressView');
        addStep('write', embedJpeg ? 'Writing XMP sidecars + embedding into JPEGs' : 'Writing XMP sidecar files', 'running');
        try {
          const res = await window.pywebview.api.write_xmp_metadata(rootPath, payload, false, false, fieldSelection, embedJpeg);
          if (!res.success) {
          ...
          if (res.skipped_conflicts && res.skipped_conflicts.length > 0) {
            ...
            dlg.querySelector('#wmOverwrite').addEventListener('click', async () => {
              ...
              try {
                const res2 = await window.pywebview.api.write_xmp_metadata(rootPath, payload, true, false, fieldSelection, embedJpeg);
                if (!res2.success) {
                  setStep('overwrite', 'failed', res2.error || 'Unknown error');
                } else {
                  setStep('overwrite', 'done', `${res2.written} file${res2.written === 1 ? '' : 's'} written${embedSuffix(res2)}`);
                  showToast(`Metadata written: ${res2.written} file${res2.written === 1 ? '' : 's'}${embedSuffix(res2)}`, 4000);
                }
              } catch (e) {
            ...
            setStep('write', 'done', `${res.written} file${res.written === 1 ? '' : 's'} written${embedSuffix(res)}`);
            showToast(`Metadata written: ${res.written} file${res.written === 1 ? '' : 's'}${embedSuffix(res)}`, 4000);
            dlg.querySelector('#wmProgressActions').style.display = 'flex';
          }
        } catch (e) {
