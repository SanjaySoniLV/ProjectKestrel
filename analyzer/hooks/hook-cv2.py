...
# macOS/Linux: Collect binaries explicitly to avoid recursion errors
# This addresses PyInstaller recursion issues with cv2 on macOS
if not compat.is_win:
    print("=" * 80, flush=True)
    print("CUSTOM CV2 HOOK: Starting cv2 binary collection for macOS/Linux", flush=True)
    print("=" * 80, flush=True)
    ...
    try:
        extra_binaries = hookutils.collect_dynamic_libs('cv2', search_patterns=['cv2*.so', 'cv2*.pyd', 'cv2*.dylib'])
    ...
    print(f"CUSTOM CV2 HOOK: Total binaries collected: {len(binaries)}", flush=True)
    print("=" * 80, flush=True)
...
print("CUSTOM CV2 HOOK: Collecting submodules...", flush=True)
...
print("CUSTOM CV2 HOOK: Collecting data files (config files)...", flush=True)
